...
if ((double)$preco <= 0.0) {
	...
	$msg .= "Veiculo ID ($idveiculo), placa($placa) esta sem o pre�o, por favor revisar o XML($arquivo)";
	Logger::logerror($msg);
	die($msg);
}
...
if (trim((string) $condpagamento) == "") {
	$msg = print_r($_POST, TRUE) . "\n";
	$msg .= "Veiculo ID ($idveiculo), placa($placa) condi��o pagamento em branco, por favor revisar o XML($arquivo)";
	Logger::logerror($msg);
	die($msg);
}
...
if ($idQueBarato) {
	...
	for ($i=0;$i<$imagecount;$i++) {
		...
		if (!$file_down) {
			Logger::logwarn("\nXML($arquivo). Imagem n�o adicionada AnuncioID: $idQueBarato, Titulo: $titulo. Imagem: $image_name");
		} else {
